...
class Student {
  ...
  static getStudentAggregated = (studentId) => {
    return db()
      ...
      .aggregate([
        ...
        {
          $project: {
            "realClass.students": 0,
            "realClass._id": 0,
            "realClass.schedule": 0,
          },
        },
      ])
      .next();
  ...
  static updateStudent = (studentId, updatedStudent) => {
    return db()
      .collection(collectionName)
      .updateOne({ _id: new ObjectId(studentId) }, { $set: updatedStudent });
  };

  static updateStudentWithConfigs = (studentId, configuration) => {
    return db()
      .collection(collectionName)
      .updateOne({ _id: new ObjectId(studentId) }, configuration);
  };
  ...
  static updateStudentsWithConfigs = (filteringObj, updatingObj) => {
    return db()
      .collection(collectionName)
      .updateMany(filteringObj, updatingObj);
  };
}
