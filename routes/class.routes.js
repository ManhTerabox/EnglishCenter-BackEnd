...
router.post(
  ...
  [
    body(
      "name",
      "Class name must be consisting of one capital character and one number for better experience"
    )
      .trim()
      .isUppercase()
        ...
        try {
          ...
          if (foundClass) {
            return Promise.reject(
              "This name is taken by some class, insert an alternative."
            );
          }
          return true;
        } catch (error) {
    ...
    body("description", "Description must contain chars only")
      .trim()
      .isString(),
  ],
...
router.patch(
  "/class/removeStudent",
  ClassControllers.patchremoveStudentFromClass
);
