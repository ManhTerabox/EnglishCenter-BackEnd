...
router.post(
  "/teachers",
  [
    body("firstName", "FirstName must contain chars only")
      .trim()
      .isString()
      .notEmpty(),
    body("lastName", "LastName must contain chars only")
      .trim()
      .isString()
      .notEmpty(),
    ...
    body("email")
      ...
      .custom(async (value, { req }) => {
        try {
          const foundTeacher = await Teacher.getTeacherWithCondition({
            email: value,
          });
          if (foundTeacher) {
            return Promise.reject("This email is taken by some teacher");
          }
          const foundStudent = await Student.getStudentWithCondition({
            email: value,
          });
...
router.delete(
  ...
  TeacherControllers.deleteDeleteTeacher
);
